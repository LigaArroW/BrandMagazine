...
const RecomendedProducts: React.FC<IRecomendedProductsProps> = ({ id, isCart = false }) => {
    ...
    return (
        <div className="mb-[25px] md:mb-[68px] xl:mb-[47px] 2xl:mb-[64px]">
            <h4 className="uppercase font-[600] text-2xl 2xl:text-[32px] inline-block text-heavyGray mb-[30px] md:mb-[42px] xl:mb-[49px] 2xl:mb-[67px]">
                Вас также может заинтересовать
            </h4>
            {recomendedProducts && recomendedProducts.recommended_products.length > 0 ? (
                <div className="grid gap-[45px] grid-cols-2 xl:grid-cols-4">
                    {recomendedProducts.recommended_products.map((product, index) => {
                        return (
